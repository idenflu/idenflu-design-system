...
테마와 무관한 기본 색상 값을 정의하며, `themes`의 시맨틱 토큰이 `{Base.[Color].[Level]} → {Base.Gray.100}` 형태로 참조합니다.
...
2. **데이터 중심의 단계적 확장**
   ...
   각 컬러 패밀리는 `100`부터 `900`까지 균등한 명도 축을 기준으로 나누어져 있으며, 이는 **의미와 상태 전달**을 우선하여 사용자가 **_명확한 시각적 위계(Visual Hierarchy)_** 를 인지할 수 있도록 돕습니다.
...
1. **Anchor(`500`) 기준과 확장성**
   ...
   Tailwind의 `[color]-500`을 기반으로 하되, 엔터프라이즈 환경에 맞게 재보정한 색상을 사용합니다.
   - 명도 기준
     - `Gray 500` 인근의 중간 명도
   ...
   - 인터랙션 확장성
     ...
     - `600` / `700` 방향으로 호버 및 액티브(Hover/Active) 상태 등
   - 접근성
...
- 색상 토큰 사용은 지양하고 시맨틱 토큰 사용 (`text-primary`)
- 인터랙션 정의 시 `Anchor(500)`를 기준으로 상하 단계를 규칙적으로 매핑  
  예: Normal=`Blue 500` → Hover=`Blue 500` → Active=`Blue 600`
- 1차 대비 검토 후 실측 검증
