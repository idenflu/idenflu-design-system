...
>(
  (
    ...
    const indexOfValue = (val: string) =>
      visibleOptions.findIndex((o) => o.value === val);

    const openPanel = () => {
      const start = selected.length
        ? indexOfValue(selected[0])
        : firstEnabled();
      setActiveIndex(
        start >= 0 && !visibleOptions[start]?.disabled ? start : firstEnabled()
      );
      setOpen(true);
    };
    ...
    const optionId = (val: string) =>
      `${baseId}-option-${val.replace(/[^a-zA-Z0-9_-]/g, "-")}`;
    ...
    const renderTriggerValue = () => {
      if (selected.length === 0)
        return (
          <span className="if-select__placeholder">
            {placeholder ?? "Select"}
          </span>
        );
      if (multiple) {
        ...
        return (
          <span className="if-select__tags">
            ...
            {extra > 0 ? (
              <span className="if-select__more">+{extra}</span>
            ) : null}
          </span>
        );
      }
      ...
      return (
        <span className="if-select__single">
          {only?.icon != null ? (
            <span className="if-select__option-icon" aria-hidden="true">
              {only.icon}
            </span>
          ) : null}
          {only?.label}
    ...
    return (
      <div
        className={classNames("if-select", multiple && "if-select--multiple")}
      >
        <div
          ref={setTriggerRef}
          role="combobox"
        ...
        {open ? (
          <div ref={panelRef} className="if-select__panel">
            ...
            <ul
              ...
              className="if-select__options"
            >
              {visibleOptions.length === 0 ? (
                <li className="if-select__empty" role="presentation">
                  결과 없음
                </li>
              ) : (
                visibleOptions.map((option, index) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className="if-select__option-check"
                        aria-hidden="true"
                      >
                      ...
                      {option.icon != null ? (
                        <span
                          className="if-select__option-icon"
                          aria-hidden="true"
                        >
                          {option.icon}
                        </span>
                      ...
                      <span className="if-select__option-text">
                        <span className="if-select__option-label">
                          {option.label}
                        </span>
                        {option.description != null ? (
                          <span className="if-select__option-description">
                            {option.description}
                          </span>
                        ) : null}
                      </span>
