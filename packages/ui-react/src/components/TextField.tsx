...
export const TextField = React.forwardRef<HTMLInputElement, TextFieldProps>(
  (
    ...
    return (
      <label className={classNames("if-field", `if-field--${fieldState}`, `if-field--${visualAvailability}`, className)} htmlFor={inputId}>
        <span className="if-field__label">
          {label}
          {required ? <em className="if-field__required">Required</em> : null}
        </span>
        {hasShell ? (
          <span className={classNames("if-input-shell", (isDisabled || disabled) && "is-disabled")}>
